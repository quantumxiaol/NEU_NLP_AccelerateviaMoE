...
class Transformer(nn.Module):
    ...
    def forward(self, enc_input, dec_input, enc_output=None, expChoice = None):
        device = self.device
        ...
        if expChoice is not None:
            dec_output, out2, actiRate = self.decoder(dec_input, decoder_self_mask, enc_output, encoder_pad_mask, expChoice)
        else:
        ...
        if expChoice is not None:
            return enc_output, result, out2, actiRate
        else:
...
class Decoder(nn.Module):
    ...
    def forward(self, dec_inputs, self_mask, enc_output, encoder_mask, expChoice = None):
        src = self.dec_embedding(dec_inputs) * math.sqrt(self.d_model)  # (bs,len,d_model)
        src = self.pos_encoder(src)
        src = self.dropout(src)
        layerINdex = 0
        for layer in self.layers:
            if layerINdex == 0 and expChoice is not None:
                src, out2, actiRate = layer(src, self_mask, enc_output, encoder_mask, expChoice)
            else:
                src = layer(src, self_mask, enc_output, encoder_mask)
            layerINdex += 1
        if expChoice is not None:
            return src, out2, actiRate
        else:
            return src
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, self_mask, enc_output, enc_pad_mask, expChoice = None):
        # norm
        res, x = x, self.norm1(x)
        # self attention
        ...
        # ffn layer
        if expChoice is not None:
            ffn_out, actiRate= self.ffn(out2, expChoice)
        else:
...
class PoswiseFeedwardNet(nn.Module):
    ...
    def forward(self, inputs, expChoice = None):
        ...
        if expChoice is not None:
            ...
            for i in range(andVector.shape[1]):
                ...
                for exp in expChoice:
                    expVector = andVector[:, i, exp]
                    actiRate[i, index] = torch.sum(expVector) / expVector.numel()
                    index += 1
            actiRate = torch.nn.functional.softmax(actiRate, dim=-1)
        output = self.fc[2](output)
        if expChoice is not None:
            return output, actiRate
        else:
            return output